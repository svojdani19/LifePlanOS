...
const toRow = (a: ReasoningAssessment) => ({
  ...
  generatedByModel: "deterministic-reasoning-v3",
});
...
export async function persistCaseReasoning(caseId: string, firmId: string, opts: PersistOptions = {}) {
  ...
  for (const it of items) {
    ...
    try {
      ...
      if (prior.materialHash === a.materialHash && prior.status !== "ERROR" && prior.reasoningChain != null && prior.generatedByModel === "deterministic-reasoning-v3") continue; // cache hit (recompute once per methodology version)
